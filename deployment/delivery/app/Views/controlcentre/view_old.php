...
//    });
    ...
    map = new ol.Map({
        ...
        layers:bingLayers.concat(vectoLayer,trackLayer,locateLayer,vectorPOI,vector,vectorGeo,vectorRoute,vectorRuler,vectorAlert,trackonHPLayervars,vectorAgps,trackonHPLayervars1),
...
<script>
    ...
    $("#saveGeofencing").on('hidden.bs.modal',function () {
		...
		if(geofencingmap){
			...
			geofencingStoreData = {};
			geoFenceCoordinates = [];
			geofencingmap.unByKey(geofencingmapclick);
			geofencingmap.unByKey(geofencingmappointermove);
			//geofencingmap.setTarget(null);				
			vector.getSource().clear();
			vectorGeo.getSource().clear();
			drawpoly = null;
...
//    var proxirouteTo    = $("[id^='proxirouteTo_']");
    ...
    $("#configurealert").on('shown.bs.modal',function () {
        ...
        if(Object.keys(deviceAssignedAlerts.notify).length > 0 && deviceAssignedAlerts.notify != false){
         ...
         if(Object.keys(deviceAssignedAlerts.lowbattery).length > 0 && deviceAssignedAlerts.lowbattery != false){
		...
		if(Object.keys(deviceAssignedAlerts.freefall).length > 0 && deviceAssignedAlerts.freefall != false){
        ...
        if(Object.keys(deviceAssignedAlerts.nomovement).length > 0 && deviceAssignedAlerts.nomovement != false){
         ...
         if(Object.keys(deviceAssignedAlerts.overspeed).length > 0 && deviceAssignedAlerts.overspeed != false){
        ...
        if(Object.keys(deviceAssignedAlerts.nodata).length > 0 && deviceAssignedAlerts.nodata != false){
...
//        });   
	...
	function followdevice(deviceid) {
		...
		var pollLayer = new ol.layer.Tile({                                                        
			...
			visible: true,
			source: new ol.source.TileWMS({
			  url: GEOSERVER_URL+'/wrbrc/wms?service=WMS&version=1.1.1&request=GetMap',
			  params: {
					   'VERSION': '1.1.1',
					   tiled: true,
					   STYLES: '',
					   LAYERS: 'wrbrc:master_polldata'
			  }
			})
		});
