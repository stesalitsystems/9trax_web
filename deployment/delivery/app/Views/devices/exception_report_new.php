                ...
                <table class="table table-bordered" id="reportlists" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <a style="width: fit-content;" href="<?= base_url('exception-report-device') . '?download=xlsx&start_date=' . ($stdt ? date('Y-m-d', strtotime($stdt)) : date('Y-m-d')) . '&end_date=' . ($endt ? date('Y-m-d', strtotime($endt)) : date('Y-m-d')) . '&usertype=' . (($usertype) ? $usertype : 'all'); ?>" class="btn btn-success btn-block" target="_blank">
                                    <i class="fa fa-download"></i> Download XLSX
                                </a>
							</th>
                            <th>
                                <a style="width: fit-content;" href="<?= base_url('exception-report-device') . '?download=pdf&start_date=' . ($stdt ? date('Y-m-d', strtotime($stdt)) : date('Y-m-d')) . '&end_date=' . ($endt ? date('Y-m-d', strtotime($endt)) : date('Y-m-d')) . '&usertype=' . (($usertype) ? $usertype : 'all'); ?>" class="btn btn-success btn-block" target="_blank">
                        ...
                        if (count($alldata) > 0) {
                            ...
                            foreach ($alldata as $irow) {
                                echo '<tr>
                                    ...
                                    <td>'. $irow->beats_not_covered_count;
                               
                                    
                                    if (!empty($irow->not_covered_device_names)) {
                                        $not_covered_device_names = is_array($irow->not_covered_device_names) ? $irow->not_covered_device_names : explode(',', $irow->not_covered_device_names);
                                        if(count($not_covered_device_names) > 0){
                                            echo '<br>';
                                            foreach ($not_covered_device_names as $deviceName) {
                                                echo '<p>' . $deviceName . '</p>';
                                            }
                                ...
                                echo '<td>' . $irow->device_off_count;
                                if (!empty($irow->off_device_names)) {
                                    $offDeviceNames = is_array($irow->off_device_names) ? $irow->off_device_names : explode(',', $irow->off_device_names);
                                    if(count($offDeviceNames) > 0){
                                        echo '<br>';
                                        foreach ($offDeviceNames as $deviceName) {
                                            echo '<p>' . $deviceName . '</p>';
                                        }
                                ...
                                echo '<td>' . $irow->low_battery_count . '</td>
                                    <td>' . $irow->over_speed_count . '</td>
                                    <td>' . $irow->defaulter_percentage . '</td>
                                </tr>';

                                $device_off_count += $irow->device_off_count;
                                $beats_covered_count += $irow->beats_covered_count;
                                $beats_not_covered_count += $irow->beats_not_covered_count;
                                $not_allocated_count += $irow->not_allocated_count;
                                $defaulter_count += $irow->defaulter_count;
                                $over_speed_count += $irow->over_speed_count;
                                $low_battery_count += $irow->low_battery_count;
                            }
                        ?>
                        <tr>
                            <td align="right">Total</td>
                            <td><?php echo ($not_allocated_count + $device_off_count + $beats_covered_count + $beats_not_covered_count); ?></td>
                            <td><?php echo $not_allocated_count; ?></td>
