...
<div class="container-fluid">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="search-panel">
                ...
                <h4>Search</h4>              
                <form action="<?= base_url()?>traxreport/duitycompletionreport" method="post" autocomplete="off" name="frmsearch" id="frmsearch">
                    <div class="form-row">
                ...
                <div class="table-responsive">
					...
					<table class="table table-bordered" id="lists" width="100%" cellspacing="0">
						...
						<tbody class="reportlists-body">
							...
							$mdddevicename = $this->db->query("select device_name from {$sessdata['schemaname']}.master_device_setup where deviceid='".$irow->mddd."'")->getResult();
							...
							// $poledetails = str_replace(")","",$poledetails);
							// $poledetails_arr = explode("-",$poledetails);
							/*$irow->deviation_distance1 = round($irow->deviation_distance1/1000,2);
							if(strpos($irow->deviation_distance1,'-') !== false)
							{
								$irow->deviation_distance1 = str_replace("-","+",$irow->deviation_distance1);
							}
							else
							{
								$irow->deviation_distance1 = "+".$irow->deviation_distance1;
							}*/
							...
							if($irow->acting_trip != ''){
								...
								<td>'.$irow->stoppol.'</td>
								<td>'.round($irow->distancecover/1000,2).' km</td>
								<td>'.$irow->walk_org_distance_out.'</td>
								<td>'.round($irow->deviation_distance1/1000,2).' km</td>
								<td>'.$irow->status.'</td>
								<td><a href="'.site_url('controlcentre/view').'/'.$irow->mddd.'/'.date("d-m-Y", strtotime($irow->result_date)).'/'.$from_datetime.'/'.$to_datetime.'" target="_blank">History</a></td>
								</tr>';
							} else { 
								echo '<tr>
								<td>NA</td>
    ...
    <script>
		...
		$("#pway_id").on('change', function() {
			...
			<?php if($_POST) {
			?>
			populateSection('<?php echo isset($pway_id); ?>');
			<?php
			}
			?>
		...
		function excelSubmit(){
			submitFRM(BASEURL + 'traxreport/duitycompletionreportexcel');
		}		
