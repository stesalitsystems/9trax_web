...
<div class="container-fluid">
    ...
    <div class="card mb-3">
		...
		<?php
			...
			<?php }
			?>                
                <?php echo form_open("traxreport/activitytimesegment",array("autocomplete"=>"off","name"=>"frmsearch","onsubmit"=>"return submit_form();")) ?>
                    <div class="form-row">
						...
						<div class="col-xs-12 col-sm-12 col-md-3 col-lg-3" id="secdiv">
                            ...
                            <input type="hidden" name="pwi_name" id="pwi_name" value="<?php if(isset($pwi_name)) echo $pwi_name;?>"/>
						...
						<div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
							...
							<input type="input" class="form-control stdt" id="stdt" name="stdt" value="<?php echo $dt; ?>" placeholder="Date" readonly>
						</div>
                    ...
                    <table class="table table-bordered" id="reportlists" width="100%" cellspacing="0">
                        ...
                        <tbody class="reportlists-body">
                            ...
                            foreach($report_data as $report_data_each){
								...
								$map_start_date = $report_data_each[0]['result_date'];
								...
								$map_end_date = $report_data_each[0]['result_date'];
								...
								$start_time = $report_data_each[0]['start_time'];
								$map_end_time = $report_data_each[0]['end_time'];
                            ?>
                            <tr onclick="plotnewtrack('<?php echo $map_device_id;?>','<?php echo $map_start_date;?>','<?php echo $start_time;?>','<?php echo $map_end_date;?>','<?php echo $map_end_time;?>');">
								...
								<td><?php echo $report_data_each[0]['newPwI'] ?? ''; ?></td>
...
<script>
    ...
    $(document).ready(function() {
		...
		$("#res").on('click', function() {
			window.location.href = BASEURL + 'traxreport/activitytimesegment';
		});
		...
		function plottrack()
		{
			var fromDate = $.trim($("#stdt").val());
			var toDate = $.trim($("#stdt").val());
	...
	function excelSubmit(){
        if($("#stdt").val()!=""){
            submitFRM(BASEURL + 'traxreport/activitytimesegmentreportexcel');
        }else{
           alert('All search fields are required');
		   return false;
        }
