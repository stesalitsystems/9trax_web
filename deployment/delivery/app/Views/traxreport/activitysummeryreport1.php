    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="search-panel">
                ...
                <h4>Search</h4>              
                <form action="<?= base_url()?>traxreport/activitysummeryreport1" method="post" autocomplete="off" name="frmsearch" id="frmsearch">
					<?= csrf_field() ?>
					<div class="form-row">
                ...
                <div class="table-responsive">
					...
					<table class="table table-bordered" id="reportlists" width="100%" cellspacing="0">
						<thead>
							<tr>
							<th colspan="20"> Activity Summary Report From <?php echo date("d-m-Y H:i", strtotime($stdt));?> To <?php echo date("d-m-Y H:i", strtotime($endt));?></th>
							</tr>
						</thead>
						<thead>
							<tr>
						...
						<tbody class="reportlists-body">
							...
							if(count($alldata) > 0) {
								...
								foreach ($alldata as $row) {
									$deviceId = $row->imeino;
									$timetravelled = strtotime($row->timetravelled);
									...
									if (!isset($groupedData[$deviceId])) {
										...
										$groupedData[$deviceId]->endbattery = $row->endbattery; // End battery
										$groupedData[$deviceId]->totaldistancetravel = $row->totaldistancetravel;
										$groupedData[$deviceId]->timetravelled = strtotime($row->timetravelled);
									} else {
										// Update the End Time with the latest occurrence
										$groupedData[$deviceId]->actual_endtime = $row->actual_endtime;
										$groupedData[$deviceId]->endbattery = $row->endbattery;
										$groupedData[$deviceId]->totaldistancetravel += $row->totaldistancetravel;
										$groupedData[$deviceId]->timetravelled += $timetravelled;
									}
								}

								$i=1; 
								foreach ($groupedData as $irow) {
									$seconds = (int) $irow->timetravelled; // Ensure it's an integer
									...
									echo '<tr>
										<td>' .$i . '</td>
										<td>' . $irow->imeino . '</td>
										<td>' . ($irow->expected_stpole ?? 'NaN') . '-' . ($irow->expected_endpole ?? 'NaN') . '</td>
