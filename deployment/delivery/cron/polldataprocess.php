...
for($row=2;$row<=$xls->rowcount();$row++) 
{
	...
	if ((strpos($xls->val($row,8), 'TP/OHE Mast') !== false)) 
	{
		$pollname = trim($xls->val($row,7));
		$latitude = convertDMSToDecimal($xls->val($row,3));
		$longitude = convertDMSToDecimal($xls->val($row,4));
		echo $insert = "insert into stes.master_polldata(name,latitude,longitude,geom) values ('".$pollname."','".$latitude."','".$longitude."',ST_GeomFromText('POINT(".$longitude." ".$latitude.")',4326))";
		echo "<br><br>";
		pg_query($insert);
	}
}
