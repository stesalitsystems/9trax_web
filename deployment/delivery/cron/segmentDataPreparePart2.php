...
$date_from = date("Y-m-d", strtotime('-1 day',strtotime(trim($curentdate)))).' 19:00:00';
$date_from1 = date("Y-m-d", strtotime('-1 day',strtotime(trim($curentdate)))).' 23:59:59';
$date_to = date('Y-m-d', strtotime($curentdate)).' 00:00:00';
$date_to1 = date('Y-m-d', strtotime($curentdate)).' 19:00:00';
...
//echo "<pre>";print_r($patrolman_time_segment_Arr);echo "</pre>";
//exit;
//================================//
...
for($dv=0;$dv<count($devices_arr);$dv++)
{
	...
	if(pg_num_rows($query_device_assignment) > 0)
	{
		if($user_type == 'Patrolman')
		{
			...
			$patrolman_activity[$patrolman_index]['serial_no'] = $serialno;
			...
			for($i=0;$i<count($patrolman_time_segment_Arr);$i++)
			{
				...
				$patrolman_activity[$patrolman_index]['time_segment_details'][$i]['avg_speed'] = number_format($row_record['avg_speed'],2);	
				$patrolman_activity[$patrolman_index]['time_segment_details'][$i]['max_speed'] = number_format($row_record['max_speed'],2);				
			}
		...
		/*else
		{
			...
			$keyman_activity[$keyman_index]['serial_no'] = $serialno;
			...
			for($i=0;$i<count($keyman_time_segment_Arr);$i++)
			{
				...
				$keyman_activity[$keyman_index]['time_segment_details'][$i]['avg_speed'] = number_format($row_record['avg_speed'],2);	
				$keyman_activity[$keyman_index]['time_segment_details'][$i]['max_speed'] = number_format($row_record['max_speed'],2);				
			}
			$sql_device_assignment_details = "select a.parent_user_id,a.current_user_id,b.organisation as pwy,c.organisation as section
											from public.device_asign_details as a
											left join public.user_login as b on (a.parent_user_id = b.user_id)
			...
			$query_device_assignment_details = pg_query($sql_device_assignment_details);
			$row_device_assignment_details = pg_fetch_array($query_device_assignment_details);
			$keyman_activity[$keyman_index]['pwy'] = $row_device_assignment_details['pwy'];
			$keyman_activity[$keyman_index]['section'] = $row_device_assignment_details['section'];
			$keyman_index++;
		}*/
	}
}
